...
- **📌 DECOMPOSIÇÃO (o P02 é grande demais para um portão só — dividido em sub-projetos gated):**
  ...
  - **P02.2 — Precificação autoritativa no servidor** — ⬜ **próxima** (decisão tomada: **plpgSQL** — sem infra nova, atômico com a criação do pedido, usa o fluxo de migrations existente; evita edge functions). Endereça a fronteira de confiança do `freight_value` forjável no `/agendar`.
  - **P02.3 — Aposentar `base44`** (276 chamadas / 55 arquivos) — ⬜ trilha longa/incremental (repositórios por módulo), projeto próprio.
  - **P02.4 — Pedido público → lead/cotação** (fim do INSERT anônimo) — ⬜ próxima versão (muda fluxo + schema).
  - **P02.5 — Reestruturação de domínios** (Order/Shipment vs Operação; Master Data vs Frota) — ⬜ adiar (organizacional, baixo valor imediato).
