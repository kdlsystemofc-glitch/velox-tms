...
### 0.4 Funcionalidades recentes — EXERCITAR OBRIGATORIAMENTE (jun/2026)
Estas existem hoje e **precisam ser testadas** (foram lacunas recém-fechadas):
1. **Prioridade operacional do pedido** (`priority`: normal/urgente/crítica) — definida em **Novo Pedido** (passo Coleta), editável no **Detalhe do Pedido**, com selo na **lista**, no **Despacho** e na separação automática. É **independente** do "Tipo de frete" (que é preço). Detalhes nas telas 2.2/2.3/2.4/2.6.
2. **Pedido parado (SLA do "novo")** — pedido em `new`/`confirmed` sem programação há mais de N dias (padrão 3, em `company_settings.stale_order_days`) vira **item na fila de exceções** da Central de Operações (2.1).
3. **Tempo de espera / estadia** — diferença entre `Confirmar Chegada` e `Concluir` em cada parada vira "tempo no local"; acima do tempo livre, gera **estadia cobrável** lançável como receita no **Detalhe da Viagem** (2.12). Tarifas em Configurações → Preços (2.24).
4. **Planejamento automático ciente de prioridade** — "Separação automática" no Despacho (2.6) ordena por prioridade operacional (crítica→urgente→normal) além de frete urgente/SLA.
5. **Fluxo de aprovação (opcional por empresa)** — toggle **"Exigir aprovação antes de operar"** em Configurações → Agendamento (2.24). Ligado: todo pedido novo entra como **"Aguardando aprovação"** e só vai à operação após um admin/operador **Aprovar** (2.4); aparece como aba/pipeline em 2.1/2.2.

**Status de pedido (completo):** `awaiting_approval` → `new` → `confirmed` → `collecting` → `in_transit` → `delivered` · `cancelled` (+ exceções `awaiting_cargo`, `partially_delivered`, `in_transfer`).
...
**Passo 5 — Revisão:** totais + valor estimado (FreightBreakdown). Erro de envio aparece neste passo. Botão final cria o pedido. **Se o fluxo de aprovação estiver ligado** (Config → Agendamento), o pedido nasce como **"Aguardando aprovação"** em vez de "Novo".
...
**Abas por status (contadores):** Todos · **Aprovação** · Novos · Confirmados · Em coleta · Em trânsito · Entregues · Cancelados.
**Tabela** (colunas ordenáveis: Protocolo/Cliente/Status...), linha clicável→`/admin/coletas/:id`, **seleção múltipla** (checkbox). Junto ao protocolo aparece o **selo de prioridade** (Crítica/Urgente; "Normal" não exibe selo) e o selo "Urg" de frete urgente. Selecionados confirmados sem viagem → barra de ação **Criar viagem**(→`/admin/viagens/nova` com `preselectedOrderIds`).
**Ações por linha:** **Novo** → **Confirmar** (modal: **Valor do frete**, **Forma de pagamento**, **Data de coleta** → grava `confirmed`, cria receita) e recusar (→`cancelled`); **Confirmado sem viagem** → **Despachar**(→despacho). **Aguardando aprovação** → abrir o pedido para **Aprovar/Recusar** (2.4).
...
No cabeçalho: **selo de status**, selo "Frete urgente" (se aplicável), e um **seletor de Prioridade** (Normal/Urgente/Crítica) que altera `priority` na hora e registra no histórico (override do operador). O **stepper** mostra a etapa "Aguardando aprovação" apenas para pedidos que passaram por aprovação.
**Ações:** **ação primária por etapa** no topo — para `awaiting_approval` é **Aprovar Pedido** (→ vira "Novo", registra "Pedido aprovado — liberado para operação"); para `new` é **Confirmar Pedido**; etc. Recusar uma aprovação = **Cancelar** o pedido (modal: "Motivo do cancelamento (obrigatório)"). Também: editar, registrar ocorrência, **Voltar**.
...
Título **Despacho** — "Selecione pedidos na fila e clique no dia/caminhão para programar". A **fila é ordenada por prioridade operacional** (crítica→urgente→normal; empate pela coleta mais antiga) e cada cartão mostra o **selo de prioridade**. Busca ("Cliente, cidade, UF...") + botão de filtro **"urgentes"** (mostra frete urgente **ou** prioridade operacional elevada). Arrastar para célula (dia × caminhão). Botão **Separação automática** → modal **"Separação automática sugerida"** com uma carga por caminhão/dia, **explicação por pedido** ("↳ por quê", citando a prioridade) e selo de prioridade; lista de **não alocados com motivo**; botões **Cancelar** / **Aplicar separação**. Célula mostra **peso · volume**; **Devolver à fila** desprograma. Botões **Limpar**, **Cancelar**.
...
**Paradas:** arrastar para reordenar + botões ↑/↓; botões **Google Maps** e **Otimizar rota** (captura km/custo estimado). Por parada: tipo (Partida/Coleta/Entrega), endereço; comboio → select de veículo; ações **Chegou**, **Concluir**; entrega → **Anexar NF** / "Ver NF Assinada". Quando há `arrived_at` e `completed_at`, mostra **"⏱ X no local"** (e "estadia cobrada" depois de lançada).
**Card Estadia / tempo de espera** (quando alguma parada excede o tempo livre): lista por parada (tempo no local · horas cobráveis · valor), **total a cobrar (pendente)** e botão **Lançar estadia como receita** (cria Revenue por pedido e marca a parada como cobrada — não cobra de novo).
...
- **Agendamento:** **Antecedência mínima** (dias úteis antes da coleta) · **Dias de operação** (Seg/Ter/Qua/Qui/Sex/Sáb/Dom) · **Fluxo de aprovação** — checkbox **"Exigir aprovação antes de operar"** (ligado: todo pedido novo entra como "Aguardando aprovação" e só vai à fila depois que um admin/operador libera; desligado: pedido entra direto como "Novo") · **Pedido parado** — campo **"dias sem programação para alertar"** (padrão 3; 0 = sinaliza imediatamente) que alimenta o alerta da Central (2.1). Botão **Salvar**.
...
- **Coleta:** botão **Confirmar Coleta** (toast "Parada concluída!"). Após concluir, mostra **"⏱ X no local"** (tempo entre chegada e conclusão).
