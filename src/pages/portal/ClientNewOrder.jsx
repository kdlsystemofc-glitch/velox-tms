...
const inputCls = "mt-1 w-full rounded-lg border border-input px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-primary/40";
const emptyItem = { nf_number: "", volumes: "", weight_kg: "" };
const emptyRecipient = () => ({ name: "", cep: "", city: "", state: "", street: "", number: "", items: [{ ...emptyItem }] });

function Field({ label, children, className = "" }) {
  return <div className={className}><label className="text-xs font-medium text-muted-foreground">{label}</label>{children}</div>;
}
...
export default function ClientNewOrder() {
  ...
  if (done) {
    return (
      <div className="max-w-lg mx-auto bg-card border border-border rounded-2xl p-8 text-center">
        ...
        <h1 className="font-display text-xl font-bold text-foreground mb-1">Pedido criado!</h1>
        <p className="text-sm text-muted-foreground">Protocolo <span className="font-mono font-bold">{done}</span>. Acompanhe o andamento em Meus Pedidos.</p>
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-4">
      <button onClick={() => navigate("/portal")} className="inline-flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground"><ArrowLeft className="w-4 h-4" /> Voltar</button>
      <div>
        <h1 className="font-display text-2xl font-bold text-foreground">Novo pedido</h1>
        <p className="text-sm text-muted-foreground">Solicite uma coleta. A Velox confirma a data e o valor.</p>
      </div>
      ...
      <form onSubmit={submit} className="space-y-5">
        ...
        {form.recipients.map((r, ri) => (
          <section key={ri} className="bg-card border border-border rounded-xl p-5">
            ...
            <div className="mt-4">
              <div className="flex items-center justify-between mb-2">
                <p className="text-xs font-semibold text-muted-foreground">Itens / Notas fiscais</p>
        ...
        <button type="button" onClick={addRecipient} className="w-full border border-dashed border-input rounded-xl py-2.5 text-sm font-medium text-muted-foreground hover:border-primary/50 hover:text-primary inline-flex items-center justify-center gap-1.5">
        ...
        {estimate > 0 && (
          <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 flex items-center justify-between">
            <div>
              <p className="text-xs text-muted-foreground">Estimativa de frete</p>
              <p className="text-[11px] text-muted-foreground">Valor aproximado — a Velox confirma ao programar.</p>
            </div>
