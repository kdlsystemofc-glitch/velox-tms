...
import { Truck, Inbox, PackageCheck, LogOut } from "lucide-react";
import { supabase } from "@/api/supabaseClient";

const navItems = [
  { to: "/parceiro", label: "Ofertas", icon: Inbox, end: true },
  { to: "/parceiro/cargas", label: "Minhas Cargas", icon: PackageCheck },
];
...
export default function CarrierLayout() {
  ...
  useEffect(() => {
    supabase.rpc("my_carrier_profile").then(({ data }) => setCompany(data?.carrier_name || ""));
  }, []);
    ...
    <div className="min-h-screen bg-muted/40">
      <header className="bg-card border-b border-border sticky top-0 z-20">
        <div className="max-w-5xl mx-auto px-4 h-16 flex items-center gap-4">
          <Link to="/parceiro" className="flex items-center gap-2.5">
            ...
            <div className="leading-none">
              ...
              <span className="text-[9px] text-muted-foreground uppercase tracking-widest block">Portal da Transportadora</span>
            </div>
          </Link>
